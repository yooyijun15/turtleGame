...
current_round = 1
final_round = 20
# Step 15-1. 라운드 별 시간 설정
round_time = 30
while current_round < final_round + 1:
    # Step 15-2. 현재 시간을 초 단위로 반환
    ...
    attackers = []
    # Step 14. 난이도
    attacker_num = 1 * current_round
    create_attackers(attacker_num)
    exit_game = True

    speed = 0.025 - (0.005 * current_round)
    # Step 10-1. 메인 루프
    while exit_game:
        # Step 15-3. 남은 시간 계산
        ...
        pen.clear()
        pen.write(f"⫸ ROUND {current_round}  ⏱ {remaining_time}초    점수 : {score} ⫷", align="center", font=("Arial", 16, "bold"))
